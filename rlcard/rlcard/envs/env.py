...
from rlcard.games.doudizhu.utils import SPECIFIC_MAP, ACTION_SPACE
import torch
class Env(object):
    ''' The base Env class
    '''
    ...
    def my_init_game(self,state,player_id):
        return self.extract_state(state), player_id
    ...
    def run(self, is_training=False, seed=None):
        ...
        self.state, self.player_id = self.init_game()
        # Loop to play the game
        trajectories[self.player_id].append(self.state)
        while not self.is_over():
            # Agent plays
            if not is_training:
                action = self.agents[self.player_id].eval_step(self.state,self.player_id)
            else:
                action = self.agents[self.player_id].step(self.state,self.player_id)
            ...
            if isinstance(action,int):
                ...
                # Save action
                trajectories[self.player_id].append(action)

            else:

                a_b = SPECIFIC_MAP[action][0]
                a_i = ACTION_SPACE[a_b]
                next_state, next_player_id = self.strstep(action)
                # Save action
                trajectories[self.player_id].append(a_i)
        ...
        for self.player_id in range(self.player_num):
