...
with open(os.path.join(ROOT_PATH, 'games/doudizhu/jsondata/abstract_map.json'), 'r') as file:
    ABSTRACT_MAP = json.load(file, object_pairs_hook=OrderedDict)
...
cached_candidate_cards = None
cached_candidate_cards_dict = None
lock = threading.RLock()

def contains_cards(candidate, target):
    global cached_candidate_cards
    global cached_candidate_cards_dict
    ...
    if not cached_candidate_cards or cached_candidate_cards != candidate:
        lock.acquire()
        cached_candidate_cards = candidate
        lock.release()
        ...
        for card in candidate:
            cards_dict[card] += 1
        lock.acquire()
        cached_candidate_cards_dict = cards_dict
        lock.release()

    cards_dict = cached_candidate_cards_dict
